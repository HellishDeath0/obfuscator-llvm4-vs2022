...
#define LOAD32H(x, y)                                                          \
  do {                                                                         \
    const uint8_t *bytes = (const uint8_t *)(y);                               \
    (x) = ((uint32_t)bytes[0] << 24) | ((uint32_t)bytes[1] << 16) |            \
...
#define LOAD64H(x, y)                                                          \
  do {                                                                         \
    const uint8_t *bytes = (const uint8_t *)(y);                               \
    (x) = ((uint64_t)bytes[0] << 56) | ((uint64_t)bytes[1] << 48) |            \
...
#ifndef ROTLc
#define ROTLc(x, n) (((x) << (n)) | ((x) >> (32 - (n))))
#endif

#ifndef ROTRc
#define ROTRc(x, n) (((x) >> (n)) | ((x) << (32 - (n))))
#endif
...
bool CryptoUtils::prng_seed() {
  static_assert(sizeof(key) >= 16, "Key buffer too small");

  try {
    // Используем системный генератор случайных чисел
    std::random_device rd;

    // Проверяем, что генератор использует аппаратную энтропию
    if (rd.entropy() < 0.001) {
      errs() << "WARNING: Using non-hardware based random generator\n";
    }

    // Генерируем 128 бит (16 байт) случайных данных
    for (size_t i = 0; i < 16; i += sizeof(uint32_t)) {
      // Получаем 32 бита случайных данных
      uint32_t rand_val = rd();

      // Копируем в буфер ключа по частям
      size_t bytes_to_copy = std::min(sizeof(uint32_t), 16 - i);
      std::memcpy(reinterpret_cast<uint8_t *>(key) + i, &rand_val,
                  bytes_to_copy);
    }
    ...
    errs() << "Random generator failed: " << e.what() << "\n";
    return false;
  }
}
...
void CryptoUtils::aes_encrypt(char *out, const char *in, const uint32_t *ks) {
  ...
  STORE32H(state0, out + 0);
  STORE32H(state1, out + 4);
  STORE32H(state2, out + 8);
  STORE32H(state3, out + 12);
}
